...
class FormatAsciiTest(tk.Frame):
	...
	def create_columns(self, descfilename, data):
		self.sel_col = []
	...
	def preparelist(self, toplevel, descfilename):
		...
		tk.Button(self, text='Ok', command=lambda: self.create_columns(descfilename, data)).pack(side="top", fill="both",**button_opt)
